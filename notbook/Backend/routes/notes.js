...
res.json(save_N)
    ...
    console.error(e.message);
    res.status(500).send(" esrrer occuerd (addnote)")
}
})
...
//new note object 
const new_N = {}
if(title){new_N.title = title}
if(desc){new_N.desc = desc}
if(tag){new_N.tag = tag}
// find and update 
...
res.json({"succcess":"note deleted", note:note})
    ...
    console.error(e.message);
    res.status(500).send(" esrrer occuerd (deletenote)")
}
})
